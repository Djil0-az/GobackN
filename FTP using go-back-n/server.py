...
def main():
    ...
    while (True):
        ...
        if option == "send":
            lock = threading.Lock()
            lock.acquire()
            filename = input("Please input the filename you want to send: ")
            ...
            send_thread = threading.Thread(target=host.send, args=(
                sock, filename, IP_PORT, RECEIVER_IP_PORT))
            send_thread.start()
            send_thread.join()
            ...
            lock.acquire()
            filename = input(
                "Please input the new filename you want to save: ")
            lock.release()
            receive_thread = threading.Thread(
                target=host.receive, args=(sock, filename, IP_PORT))
